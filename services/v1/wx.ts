...
async function signUrl(ctx) {
  let jsapi_ticket = await redis.get('jsapi_ticket')
  // 如果jsapi_ticket过期
  if (!jsapi_ticket) {
    ...
    if (!re.data.errcode) {
      if (re.data.access_token) {
        ...
        if (re2.data.errcode === 0) {
          jsapi_ticket = re2.data.ticket
          redis.set('jsapi_ticket', re2.data.ticket, re2.data.expires_in)
        } else {
  ...
  let str = `jsapi_ticket=${jsapi_ticket}&noncestr=${body.nonceStr}&timestamp=${body.timestamp}&url=${body.url}`
